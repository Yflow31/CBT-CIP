...
class Speed : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FromSpinner.adapter = ArrayAdapter.createFromResource(this,R.array.Speed_items,android.R.layout.simple_spinner_item).also {
            it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        }
        ToSpinner.adapter = ArrayAdapter.createFromResource(this,R.array.Speed_items,android.R.layout.simple_spinner_item).also {
            it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        }
    ...
    private fun calculate(selectedFrom: String, selectedTo: String) {
        ...
        val result = when {
            selectedFrom == "Miles per hour" && selectedTo == "Kilometers per hour" -> input * 1.609
            selectedFrom == "Miles per hour" && selectedTo == "Meters per second" -> input * 0.447
            selectedFrom == "Miles per hour" && selectedTo == "Miles per hour" -> input

            selectedFrom == "Kilometers per hour" && selectedTo == "Miles per hour" -> input * 0.621
            selectedFrom == "Kilometers per hour" && selectedTo == "Meters per second" -> input * 0.278
            selectedFrom == "Kilometers per hour" && selectedTo == "Kilometers per hour" -> input

            selectedFrom == "Meters per second" && selectedTo == "Miles per hour" -> input * 2.237
            selectedFrom == "Meters per second" && selectedTo == "Kilometers per hour" -> input * 3.6
            selectedFrom == "Meters per second" && selectedTo == "Meters per second" -> input
